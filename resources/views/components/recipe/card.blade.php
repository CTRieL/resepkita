...
<div onclick="window.location='{{ route('recipe.show', $recipe->id) }}'" tabindex="0" role="button" class="relative slide-in-up bg-white rounded-3xl cursor-pointer shadow border border-gray-100 flex flex-col gap-2 p-4 w-full transition hover:shadow-lg hover:scale-[1.015] focus:outline-none">
    {{-- modifier btn --}}
    ...
    <div class="flex items-center gap-2">
        <p class="flex items-center gap-2 px-3 py-1 transition-all">
            @if($recipe->user && $recipe->user->photo_path)
                <img src="{{ asset('storage/' . $recipe->user->photo_path) }}" alt="Foto Profil" class="w-8 h-8 rounded-full object-cover border border-gray-300">
            @else
            ...
            <div class="flex flex-col">
                <span class="font-semibold text-sm p-0">{{ $recipe->user ? $recipe->user->name : '' }}</span>
                <span class="text-gray-600 text-[10px] p-0">{{ $recipe->created_at->diffForHumans() }}</span>
            </div>
        </p>
    </div>

    <div class="recipe-card grid grid-cols-7 gap-2 relative items-center">

        {{-- img section --}}
        <div class="col-span-2 w-full aspect-square rounded-lg overflow-hidden flex flex-col items-center justify-center">
        ...
        <div class="col-span-5 w-full h-full pl-3 pr-5 flex flex-col gap-2">
            ...
            <div class="flex items-center gap-2 mt-1 text-primary justify-end">
                @php
                    $user = auth()->user();
