        ...
        <form method="POST" action="{{ route('recipe.store') }}" enctype="multipart/form-data" class="grid grid-cols-5 gap-8">
                ...
                <div id="drop-area" class="flex justify-center rounded-lg p-4 pb-8 border border-dashed border-primary/40 transition-colors duration-200 bg-primary/5">
                    <div class="text-center w-full flex justify-center items-center flex-col">
                        <div id="photo-preview" class="w-[80%] aspect-square flex justify-center items-center">
                            <svg id="svg-placeholder" class="mt-10 text-gray-300 {{ old('photo') ? 'hidden' : '' }}" height="80px" width="80px" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" data-slot="icon">
            ...
            <div class="col-span-3 flex flex-col gap-4">
                <div class="grid grid-cols-2 gap-4 cursor-default">
                    <div>
                        <input type="radio" id="private" name="privacy" value="private" class="accent-primary mr-2 cursor-pointer" @checked(old('privacy', 'private') == 'private')>
                    ...
                    <div>
                        <input type="radio" id="public" name="privacy" value="public" class="accent-primary mr-2 cursor-pointer" @checked(old('privacy') == 'public')>
                ...
                <input name="title" type="text" value="{{ old('title') }}" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">

                <label class="font-semibold mt-2">Deskripsi</label>
                <textarea name="description" rows="2" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">{{ old('description') }}</textarea>

                <label class="font-semibold mt-2">Bahan-Bahan</label>
                <textarea name="ingredients" rows="3" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">{{ old('ingredients') }}</textarea>

                <label class="font-semibold mt-2">Langkah-Langkah</label>
                <textarea name="directions" rows="4" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">{{ old('directions') }}</textarea>

                <button type="submit" class="mt-6 bg-primary text-white font-semibold py-3 rounded-xl hover:bg-primary/90 transition text-base shadow">Tambahkan</button>
            </div>
        </form>
    </div>
</div>
