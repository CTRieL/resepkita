...
<div class="w-full flex justify-center items-center py-8">
    <div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-4xl border border-gray-100">
        <h2 class="text-xl font-bold text-primary mb-2">Edit Resep</h2>
        @if($errors->any())
        ...
        @endif
        <form method="POST" action="{{ route('recipe.update', $resep->id) }}" enctype="multipart/form-data" class="grid grid-cols-5 gap-8">
            @csrf
            @method('PUT')
                ...
                <div id="drop-area" class="flex justify-center rounded-lg p-4 pb-8 border border-dashed border-primary/40 transition-colors duration-200 bg-primary/5">
                    <div class="text-center w-full flex justify-center items-center flex-col">
                        <div id="photo-preview" class="w-[80%] aspect-square flex justify-center items-center">
                            @if($resep->photo_path)
                                <img src="{{ asset('storage/' . $resep->photo_path) }}" alt="Foto Resep" class="mx-auto rounded-lg object-cover max-h-32 max-w-32 border border-gray-200" />
                            @else
            ...
            <div class="col-span-3 flex flex-col gap-4">
                <div class="grid grid-cols-2 gap-4 cursor-default">
                    <div>
                        <input type="radio" id="private" name="privacy" value="private" class="accent-primary mr-2 cursor-pointer" @checked(old('privacy', $resep->privacy) == 'private')>
                    ...
                    <div>
                        <input type="radio" id="public" name="privacy" value="public" class="accent-primary mr-2 cursor-pointer" @checked(old('privacy', $resep->privacy) == 'public')>
                ...
                <input name="title" type="text" value="{{ old('title', $resep->title) }}" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">

                <label class="font-semibold mt-2">Deskripsi</label>
                <textarea name="description" rows="2" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">{{ old('description', $resep->description) }}</textarea>
                
                <label class="font-semibold mt-2">Bahan-Bahan</label>
                <textarea name="ingredients" rows="3" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">{{ old('ingredients', $resep->ingredients) }}</textarea>
                
                <label class="font-semibold mt-2">Langkah-Langkah</label>
                <textarea name="directions" rows="4" class="block w-full py-2 px-3 border border-gray-300 rounded-lg text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40">{{ old('directions', $resep->directions) }}</textarea>
